...
### Лёгкий планировщик
`LightPlanExecutor` формирует минимальный план с фиксированной целью и набором шагов: создание курсора (`keyword_cursor-create_keyword_cursor` с запасным `cursor-create_cursor`), чтение батчей через `chat_cursor_tools-read_cursor_batch`, финализация. План держит цель, номер шага и причину остановки (`goal_reached`, `no_more_batches`, `step_limit`), сериализуется в компактный JSON для логов и передаётся в контекст чата. Системный промпт подчёркивает, что план уже утверждён, поэтому агенту нужно лишь следовать шагам и указывать цель/stop reason в финальном ответе.
