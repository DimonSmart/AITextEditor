...
## Инструменты
### Навигация
- **Типы курсоров.** Сейчас есть два вида курсоров: (1) полноценный LLM-курсор, который управляется моделью через функции (`read_cursor_batch` внутри `run_chat_cursor_agent`), и (2) быстрый keyword-курсор, который фильтрует элементы по ключевым словам без участия LLM и отдаёт только совпадения.
...
### Ненужные/устаревшие функции
- `read_document` в плагине редактора читает весь документ целиком и опасна для больших файлов — помечена как устаревшая и подлежит удалению.
- Методы работы с несколькими документами (варианты `LoadDocument`, `GetItems(documentId)`, `ApplyOperations(documentId)` и т.п.) больше не нужны в однодокументном рантайме и тоже отмечены как устаревшие.
...
Ранее курсоры были именованными и жили в общей инфраструктуре (`UseWholeBookCursor`, `QueryCursor`, `MapCursor`, `DescribePointer`, статусы `found|continue|complete`). Теперь курсор создаётся локально внутри `RunCursorAgent`, и подагент работает только в рамках одного вызова. Самодельная итерация `run_agent` и старые именованные курсоры считаются **obsolete**; используйте чат-агент и keyword-курсоры.
