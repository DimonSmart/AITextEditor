...
- адресовать элементы семантическим указателем (`SemanticPointer.ToCompactString()` вида `label`, например `1.1.1.p21`);
...
- порции: `MaxElements` = 20, `MaxBytes` = 8000.
...
`SemanticPointer` состоит из метки `Label` (например, `1.2.p3`). В курсорах и в параметрах инструментов используется строковый формат `label` из `SemanticPointer.ToCompactString()`.
