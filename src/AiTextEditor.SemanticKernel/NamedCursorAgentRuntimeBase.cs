...
public abstract class NamedCursorAgentRuntimeBase<TCursor> where TCursor : INamedCursorStream
{
    ...
    private readonly string cursorType;

    protected NamedCursorAgentRuntimeBase(
        ...
        string cursorType)
    {
        ...
        ArgumentException.ThrowIfNullOrWhiteSpace(cursorType);
    ...
    public async Task<CursorAgentResult> RunAsync(string cursorName, CursorAgentRequest request, CancellationToken cancellationToken = default)
    {
        ...
        for (var step = 0; step < maxSteps; step++)
        {
            ...
            logger.LogDebug(
                ...
                cursorPortionView.HasMore ? $"{cursorType}_batch" : $"{cursorType}_batch_complete",
            ...
            if (command == null)
            {
                logger.LogError("{CursorType}_agent_response_malformed", cursorType);
    ...
    protected abstract TCursor GetCursor(string cursorName);
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogWarning("{CursorType}_finalizer_pointer_missing_or_invalid", cursorType);
            return new CursorAgentResult(
               false,
    ...
    private async Task<AgentCommand?> GetNextCommandAsync(string agentSystemPrompt, string taskDefinitionPrompt, string evidenceSnapshot, string batchMessage, CancellationToken cancellationToken, int step)
    {
        ...
        {
            logger.LogDebug(
                "{CursorType}_agent_parsed: step={Step}, action={Action}, batchFound={BatchFound}, finishFound={Finish}, parsedAction={ParsedAction}",
                cursorType,
                step,
                parsed.Action,
                parsed.BatchFound,
    ...
    private void LogCompletionSkeleton(int step, object? message)
    {
        if (message == null)
        {
            logger.LogInformation("{CursorType}_agent_call: step={Step}, model=<unknown>, tokens=<unknown>, result=<empty>", cursorType, step);
            return;
        }
        ...
        logger.LogInformation("{CursorType}_agent_call: step={Step}, model={Model}, tokens={Tokens}, result=<received>", cursorType, step, modelId, tokens);
    }

    private void LogRawCompletion(int step, string content)
    {
        var snippet = Truncate(content, 1000);
        logger.LogDebug("{CursorType}_agent_raw: step={Step}, snippet={Snippet}", cursorType, step, snippet);
        logger.LogDebug("{CursorType}_agent_raw_len: step={Step}, len={Len}", cursorType, step, content.Length);
        logger.LogDebug("{CursorType}_agent_raw_head: step={Step}, head={Head}", cursorType, step, content[..Math.Min(300, content.Length)]);
        logger.LogDebug("{CursorType}_agent_raw_tail: step={Step}, tail={Tail}", cursorType, step, content[^Math.Min(300, content.Length)..]);
    }
